...
const PostNew = () => {
  ...
  const { token, userData } = useContext(ClientContext);
  ...
  const handleSubmit = (evt) => {
    ...
    BelowDeckApi.addPost(postData, token).then(data => {
      console.log(data)
    ...
    <div className="m-8">
      <form className="space-y-8 divide-y divide-gray-200" onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input type="text" className="hidden" name="userId" id="userId" onChange={handleChange} value={userData.id} />
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              value={postData.status} onChange={handleChange} defaultValue="0"
            >
              <option id="0" value="1">Draft</option>
              <option id="1" value="1">Published</option>
            </select>
          </div>
          <PostTypeSelect onChangeHandler={handleChange} />
        </div>
